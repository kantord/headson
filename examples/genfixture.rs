...
fn main() -> anyhow::Result<()> {
    // Defaults
    let count: usize = parse_arg("--count").and_then(|s| s.parse().ok()).unwrap_or(200_000);
    let seed: u64 = parse_arg("--seed").and_then(|s| s.parse().ok()).unwrap_or(42);
    let base_url = parse_arg("--base-url").unwrap_or_else(|| "https://example.com/api/v1/item/".to_string());

    let stdout = io::stdout();
    let mut w = io::BufWriter::with_capacity(1 << 20, stdout.lock());

    // Header
    let header = Root { count, next: None, previous: None, _phantom: std::marker::PhantomData };
    write!(&mut w, "{{\"count\":{},\"next\":null,\"previous\":null,\"results\":[", header.count)?;
