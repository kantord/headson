...
pub fn render_array(template: OutputTemplate, ctx: &ArrayCtx) -> String {
    match template {
        ...
        .render()
        .unwrap(),
        OutputTemplate::Pseudo => PseudoArray {
            children: &ctx.children,
            children_len: ctx.children_len,
            omitted: ctx.omitted,
            indent0: &ctx.indent0,
...
pub fn render_object(template: OutputTemplate, ctx: &ObjectCtx) -> String {
    match template {
        ...
        .render()
        .unwrap(),
        OutputTemplate::Pseudo => PseudoObject {
            children: &ctx.children,
            children_len: ctx.children_len,
            omitted: ctx.omitted,
            indent0: &ctx.indent0,
