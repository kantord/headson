...
# Scales to multiply GEN_COUNT by. Default: 1x, 10x, 100x.
SCALES_CSV="${HF_SCALES:-1,10,100}"
...
# Iterate over scales (1x,10x,100x by default)
IFS=',' read -r -a SCALES <<< "$SCALES_CSV"
for scale in "${SCALES[@]}"; do
  COUNT=$(( GEN_COUNT * scale ))
  EX_FILE="examples/bench_fixture_${COUNT}.json"
  TMP_WRITE="examples/bench_write_tmp_${COUNT}.json"

  echo
  echo "=== Dataset: COUNT=$COUNT (scale=${scale}x) ==="

  # 1) Pipe scenario
  OUT_PIPE_JSON="$OUT_DIR/bench_pipe_${COUNT}_${STAMP}.json"
  echo "Running hyperfine (PIPE) with $COUNT items, seed=$GEN_SEED, budgets [$BUDGETS_STR], template '$TEMPLATE'..."
  hyperfine \
    --warmup "$WARMUP" \
    --runs "$RUNS" \
    --export-json "$OUT_PIPE_JSON" \
    --parameter-list n "$BUDGETS_STR" \
    "target/release/examples/genfixture --count $COUNT --seed $GEN_SEED | target/release/headson --profile -n {n} -f $TEMPLATE > /dev/null"

  # 2) File scenario
  echo "Generating file fixture ($COUNT items) at $EX_FILE ..."
  mkdir -p "examples"
  target/release/examples/genfixture --count "$COUNT" --seed "$GEN_SEED" > "$EX_FILE"
  BYTES=$(wc -c < "$EX_FILE" | tr -d ' ')
  if command -v du >/dev/null 2>&1; then HR=$(du -h "$EX_FILE" | cut -f1); else HR="${BYTES}B"; fi
  echo "Fixture size: $BYTES bytes ($HR)"

  OUT_FILE_JSON="$OUT_DIR/bench_file_${COUNT}_${STAMP}.json"
  ...
  OUT_WRITE_JSON="$OUT_DIR/bench_write_${COUNT}_${STAMP}.json"
  echo "Running hyperfine (GEN → /dev/null)..."
  hyperfine \
    --warmup "$WARMUP" \
    --runs "$RUNS" \
    --export-json "$OUT_GEN_JSON" \
    "target/release/examples/genfixture --count $COUNT --seed $GEN_SEED > /dev/null"
  ...
  hyperfine \
    ...
    "target/release/examples/genfixture --count $COUNT --seed $GEN_SEED > $TMP_WRITE"
